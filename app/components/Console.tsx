...
const Console: React.FC = () => {
  ...
  const [line, setLine] = useState<
    Array<{ command: string; response: React.ReactNode }>
  >([]);
  ...
  const handleSubmit = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.currentTarget.value === "cv" || e.currentTarget.value === "resume") {
      ...
      link.href =
        "https://drive.google.com/uc?export=download&id=1uOZcxKFGFqUL7dwcs3otMgPVGZo4t_eD";
  ...
  return (
    <div className="sm:p-10 p-4 w-full min-h-screen cursor-default">
      ...
      {line.map((entry, index) => (
        <div
          key={index}
          className="whitespace-pre-line break-words animate-typewriter"
        >
          ...
          <div className="sm:ml-[11rem] whitespace-pre-line break-words">
            {entry.response}
          </div>
        </div>
      ))}
